...
from utils.utils import print_warning, to_dict
...
from utils.utils import print_log, print_debug, print_sucess
from exceptions.ApiToMutchInstantiationException import ApiToMutchInstantiationException
...
class Api(Task):
    ...
    def __init__(self):
        ...
        print("host : "+str(self.__host) + ", port : " + str(self.__port))
    ...
    def execute(self):
        ...
        def make_research_from_client():
            """Function which allow web client to launch research using Opse core"""
        ...
        @app.route('/get_visibility', methods=['GET'])
        def get_visibility():
            """
            Function which get visibility of attribute in one  Research
            This function show attribute user you to see for his research.
            """
            ...
            try:
                ...
                research = Research.get_research(data["research_id"])
                return jsonify(research.get_profiles_visibility()), 200

            except Exception as e:
                print_log(str(e) + "\n" + str(traceback.format_exc()))
                return self.send_error_message("Erreur interne. Impossible de traiter la demande", 500)

        @app.route('/set_visibility', methods=['PUT'])
        def set_visible():
            ...
            try:
                ...
                for key in data:
                    if key == "research_id" or key == "uuid":
                        continue
                ...
                research = Research.get_research(data["research_id"])
                map_visibility: dict = research.get_profiles_visibility()
                for key in data:
                    if key in map_visibility.keys():
                        map_visibility[key] = data[key]
